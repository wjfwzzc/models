...
def make_parser():
    ...
    parser.add_argument(
        "-n", "--n_gpus", default=1, type=int, help="total number of gpus for training",
    )
    parser.add_argument(
...
def main():
    ...
    # ------------------------ begin training -------------------------- #
    logger.info("Device Count = %d", args.n_gpus)
    ...
    if args.n_gpus > 1:
        trainer = dist.launcher(worker, n_gpus=args.n_gpus)
        trainer(args)
    else:
        worker(args)
